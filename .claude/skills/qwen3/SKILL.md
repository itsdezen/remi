---
name: qwen3
description: Use for trivial, cheap, or high-volume sub-tasks (classification, extraction, short summarization, simple text formatting/rewriting, boilerplate drafts, quick lookups) that don't need the main cloud agent's intelligence — offload to the local Ollama qwen3:8b model to save tokens/cost. Not for coding, complex reasoning, multi-step agentic work, or anything user-facing where quality matters — those stay on the main agent or go through herdr.
---

# qwen3 (local, via Ollama)

Local model, not a coding-agent CLI — no `herdr` involvement. Talk to it directly over Ollama's HTTP API on `localhost:11434`, or `ollama run qwen3:8b "<prompt>"` for a one-off manual check.
...
- Short summarization of small inputs (well under the 40K context — don't feed it huge documents)
...
## When NOT to offload here
- Anything involving code editing, multi-file reasoning, or agentic tool use — that's the main cloud agent's job, or a `herdr`-spawned coding agent for parallel work. qwen3 has a "tools" capability flag but it isn't proven reliable enough here to trust for real actions.
- Complex reasoning, math, or planning — thinking mode exists for this but is impractically slow on this hardware (see above); just use the cloud agent.
...
- Long inputs — 40K context is generous for a local model but tiny next to cloud models; don't dump large files into it.
