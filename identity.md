...
Progressive disclosure is the standard for every managed project's context files — `AGENTS.md` as sole entry point, universal across coding agents; `CLAUDE.md` on top only where Claude is used, kept to a single `@AGENTS.md` line. Rules inside stay clause/block-form and minimal, never rambling. Apply the same rigor to every managed project that this file applies to itself:
