...
## Current focus
2026-08-15: Fantasque Sans Mono Nerd Font added as the new primary font, Ghostty (`ghostty/.config/ghostty/config`, the only place a font family is set — nvim/starship/btop/lazygit/superfile all inherit the terminal font) now uses `FantasqueSansM Nerd Font Mono`. Brewfile got the new cask (`font-fantasque-sans-mono-nerd-font`); `font-maple-mono-nf` deliberately kept installed for quick switching back, per user request. Validated via `stow -n` dry-run and `brew bundle check`. Committed locally (5ff90fd), **not pushed** — awaiting push confirmation. Done via delegated herdr sub-agent, babysat in default permission mode.
