...
## Current focus
2026-08-16: CLAUDE.md content moved into a new universal AGENTS.md (genericized: title/self-references and rule-5 doc-sync table renamed from CLAUDE.md to AGENTS.md throughout); CLAUDE.md reduced to a single `@AGENTS.md` pointer line. Not committed (left for user review — this session's task didn't include auto-commit for dotfiles specifically). Done via a delegated herdr staff session, babysat in default permission mode.

2026-08-15 (cont. 3): Language runtime brand colors brightened for fg-only visibility (old colors were designed as pill backgrounds, too dark/muted as plain text on transparent bg). node_green #215732→#5FA04E, bun_black #000000→#FBF0DF (Bun's cream/tan brand accent — user tried plain white #FFFFFF first, then settled back on cream), python_blue #306998→#4B8BBE, rust_orange #B7410E→#F74C00, go_blue #375EAB→#00ADD8 (current official Gopher cyan). Amended into 3e944f6, not pushed. Note: mid-session a "down+Enter" sent to reject a permission dialog accidentally landed on "Yes, allow all edits during this session" instead and flipped the staff pane into auto-accept-edits mode — the shift+tab classifier block (identity.md) applies in reverse too, couldn't flip it back. Proceeded anyway since remaining scope was safe (no push, confined to this file); worth remembering that arrow-key dialog navigation can misfire into the auto-accept option, not just literal shift+tab.
