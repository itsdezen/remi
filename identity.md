...
## Design principle
Progressive disclosure, everywhere: entry points stay minimal (name + one-line description always visible), full detail loads only when a task actually calls for it. Governs this file's own structure (short, single-purpose sections) and skills (`SKILL.md` frontmatter vs. body). Extends to context itself, not just skills: a project's nested `AGENTS.md` files load only when that subdirectory is actually touched — never preload a project's whole doc tree "to be safe."

New rules added anywhere in this file or under `.agents/skills/` should carry the failure they prevent, not just the instruction — a rule with no "why" invites being applied past the case it was meant for.
...
- **OpenCode's own agents**: OpenCode's built-in `primary`/`subagent` persona-switching *within one opencode CLI process* (its Build/Plan primaries, or custom agents like `debug`/`review` defined in its own config). A persona change inside a single running staff member, not a new process. _Avoid_: reading "the opencode agent switched" as a new Staff appearing, or as Claude Code's Sub-agent tool — a `--kind opencode` staff member changing persona is still the same herdr-tracked process throughout.
...
Progressive disclosure is the standard for every managed project's context files — `AGENTS.md` as sole entry point (plus `.agents/skills/<tool>/SKILL.md` for tool docs) is the convention for every agent, no exceptions: Claude Code, OpenCode, and Codex all read `AGENTS.md` natively, so no per-agent entry file or per-agent discovery layer is needed anywhere. Rules inside stay clause/block-form and minimal, never rambling. This file is the reference implementation of these conventions: whenever they evolve here, that evolution is the source of truth for what every managed project's context files should look like too — applied at kickstart, and re-aligned opportunistically as an already-kickstarted project is revisited, never as an unprompted mass rewrite.
...
## Tool memory
External tools I operate (herdr, etc.) live as plain markdown under `.agents/skills/<tool>/SKILL.md`, readable by any coding agent — `AGENTS.md` points to them directly, which is all any runtime ever needs. No agent gets a special-cased discovery layer of its own; one source, one entry point, for every runtime alike.
...
- Any change I make within this repo (`identity.md`, `.agents/skills/`, etc.) gets **committed and pushed automatically** — a standing, pre-authorized exception to the usual "confirm before push" rule, scoped to this repo only.
...
- **Permission handling differs by `--kind`** — the mechanics below aren't interchangeable across CLIs:
  - **`--kind claude`**: `--dangerously-skip-permissions` and mid-session escalation (e.g. `Shift+Tab` to flip a staff pane into auto-accept-edits mode) are both blocked here by Remi's own auto-mode classifier, which flags the keystroke itself, not just the flag (likely an enterprise policy) — treat both as unavailable and work within default permission mode instead. When relaying a rejection to a permission dialog with `send-keys`, send only the literal reject key(s) (e.g. the number for "No", or repeated presses of the same direction you already know lands there), since dialogs list "Yes, allow all edits during this session" (which flips the pane into auto-accept-edits mode) as one of the selectable options and a miscounted arrow press can select it by accident. If that happens, accept the resulting mode (the reverse Shift+Tab flip-back hits the same classifier block) and proceed carefully, scoped to safe/reversible work, telling the user what happened. Fallback if truly stuck: ask the user to hand-edit the target project's own `.claude/settings.json` — Remi's own Bash/Edit attempts on any settings.json hit the same classifier block.
  - **`--kind opencode`**: OpenCode's default — with no `permission` key configured anywhere — is to run `edit`/`bash` with **no prompt at all** (verified 2026-09-09: a bare `opencode` staff member created a file and ran a shell command without ever going `blocked`). This is more permissive than Claude Code's ask-by-default, so "babysit in default mode" is not actually a safety net for opencode staff unless the target project (or a custom agent's frontmatter) explicitly sets `"permission": {"edit": "ask", "bash": "ask", ...}` in its `opencode.json`/`opencode.jsonc` — check for that before treating an opencode staff member as gated. When it *is* set to `ask`, the interactive prompt offers **Allow once / Allow always / Reject**; default focus starts on "Allow once" — navigate with `right`/`left`, confirm with `enter` (`send-keys <target> right right enter` lands on Reject, verified). `--auto` is opencode's rough equivalent of `--dangerously-skip-permissions`: confirmed it fully bypasses an `ask` permission with zero prompt when passed as a forwarded CLI arg (`-- --auto`); whether Remi's own auto-mode classifier also blocks it when relayed from a live, monitored Remi session is still unverified (this test ran outside that context, from a fresh unmonitored session) — treat it as equally off-limits until proven otherwise. Fallback if truly stuck: ask the user to hand-edit the target project's own `opencode.json`/`opencode.jsonc` `permission` key (or the relevant `agents/*.md` frontmatter) — the opencode analog of `.claude/settings.json`.
...
- Chrome-extension permission dialogs (`--kind claude` only — this is Claude Code's own browser tool) surface as idle rather than `blocked` via `agent_status`/`wait`. When a staff member goes idle after anything browser-related, `agent read` it directly to check for a stuck dialog.
